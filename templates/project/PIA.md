...
7. **Each card stands alone.** Write it for a reader who has read nothing else: a short context that says what is being built and why this decision exists, 2–4 options each with its consequence, the decision and its reason, what it depends on and what it affects. *Simple to read, deep at the same time.* Plain words; file paths only when essential.
...
- **No new decisions.** If one is truly missing, add a card to `decisions.md` (ask the lead to relay it to the researcher's file ownership — or, if the researcher is gone, the planner may append it) with status `agent`, and tell the reviewer.
...
- When everything is done, tell the lead `IMPLEMENTATION DONE` with: what was built (3–5 bullets), how to test it (short numbered steps), and what you could not verify yourself.
